...
for ii, name in enumerate(files):
	...
	for i in txtList:
		...
		c_num=oneline[0]
		if dict_[c_num] is not targetObject:
			continue
		...
		# print(target_path_,(x_min,y_min,x_max,y_max),crop_img.shape,crop_img_hash)
		print(name,'Cut',crop_img_hash)
		# cv2.imwrite(target_path_,crop_img)
		showName=str(image_counter)+"_"+str(line_counter)+"_"+dict_[c_num]
		print('showName',showName,name)
		cv2.imshow(showName, cv2.resize(crop_img,(300,300)) )
		cv2.waitKey(1)
cv2.waitKey(0)
